...
def save_to_db(cursor, conn, data: Dict[str, Any]):
    ...
    vals = (
        data.get("massar", "unknown"),
        full_name,
        data.get("parcours", "unknown"),
        data.get("module", "unknown"),
        tp_val,
        exam_val,
        moy_val,
        data.get("res", "-"),
        data.get("is_rattrapage", False),
    )
...
def find_column_index(
    headers: List[str], keywords: List[str], from_right: bool = False
) -> int:
    """Finds the column index, with the ability to scan backwards to avoid duplicate header traps."""
    # Added 'è' to the replace block just in case of weird administration formatting
    clean_headers = [
        h.lower().replace("é", "e").replace("è", "e").strip() for h in headers
    ]

    search_space = list(enumerate(clean_headers))
    if from_right:
        search_space.reverse()

    for kw in keywords:
        for idx, header in search_space:
...
def parse_html_to_data(html_content: str) -> List[Dict[str, Any]]:
    ...
    for div in soup.find_all("div", class_="table-responsive"):
        ...
        # --- THE BULLETPROOF MAPPING DICTIONARY ---
        indices = {
            ...
            # 1. Added 'resultat' to the fallback keywords so it catches the missing grades
            "moy": find_column_index(
                headers,
                ["moyenne finale", "note finale", "moyenne", "moy", "note", "resultat"],
            ),
            # 2. Forced right-to-left scanning to guarantee we grab the 'V/R/NV' column
            "res": find_column_index(headers, ["resultat", "res"], from_right=True),
        }
        ...
        for row in tbody.find_all("tr"):
            ...
            def get_val(key):
                ...
                return (
                    ...
                    else ("-" if key in ["tp", "exam", "moy", "res"] else "unknown")
                )
...
def main():
    ...
    parser.add_argument(
        "-f", "--file", type=str, help="path to the text file containing massar ids"
    )
    ...
    target_file = (
        args.file
        or input(
            f"{cyan}[*] enter the name of the id file (e.g., ids.txt): {reset}"
        ).strip()
    )
    ...
    with requests.Session() as session:
        try:
            for index, student_id in enumerate(student_ids, 1):
                ...
                print(
                    f"\r{purple}[*] [{index}/{total_ids}]{reset} fetching {bold}{student_id}{reset} (eta: {m:02d}:{s:02d}) ... ",
                    end="",
                    flush=True,
                )
